...
{ 
    ...
    public partial class TiC : ContentPage
    {
        ...
        public TiC()
        {
            ...
            bool haskey = Preferences.ContainsKey("TiC");
            if (haskey)
            {
                string date = Preferences.Get("TiC", "");
                if (date != null)
                {
        ...
        private async void MyBtn_Clicked(object sender, EventArgs e)
        {
            ...
            int voturi = Convert.ToInt32(await firebase.Child("TiC/Voturi").OnceSingleAsync<int>());
            voturi = voturi + 1;
            await firebase.Child("TiC/Voturi").PutAsync(voturi);
            myBtn.IsEnabled = false;
        }
