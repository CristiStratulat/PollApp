...
namespace Concurs
{
    ...
    public partial class Panda : ContentPage
    {
        ...
        public Panda()
        {
            ...
            bool haskey = Preferences.ContainsKey("Panda");
            if (haskey)
            {
                string date = Preferences.Get("Panda", "");
                if (date != null)
                {
        ...
        private async void MyBtn_Clicked(object sender, EventArgs e)
        {
            ...
            int voturi = Convert.ToInt32(await firebase.Child("Panda/Voturi").OnceSingleAsync<int>());
            voturi = voturi + 1;
            await firebase.Child("Panda/Voturi").PutAsync(voturi);
            myBtn.IsEnabled = false;
        }
        protected override void OnDisappearing()
        {
            ...
            Preferences.Set("Panda", clickdate);
        }
    }
}