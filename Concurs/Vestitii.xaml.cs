...
namespace Concurs
{
    ...
    public partial class Vestitii : ContentPage
    {
        ...
        public Vestitii()
        {
            ...
            bool haskey = Preferences.ContainsKey("Vestitii");
            if (haskey)
            {
                string date = Preferences.Get("Vestitii", "");
                if (date != null)
                {
        ...
        private async void MyBtn_Clicked(object sender, EventArgs e)
        {
            ...
            int voturi = Convert.ToInt32(await firebase.Child("Vestitii/Voturi").OnceSingleAsync<int>());
            voturi = voturi + 1;
            await firebase.Child("Vestitii/Voturi").PutAsync(voturi);
            myBtn.IsEnabled = false;
        }
        protected override void OnDisappearing()
        {
            ...
            Preferences.Set("Vestitii", clickdate);
        }
    }
}